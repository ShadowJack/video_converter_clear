...
<html>
    ...
    <body>
        ...
        **/
        ...
        elseif ( count($args) == 1 && $args[0] == 'new' && $method == 'GET' )      // matches GET: /videos/new
        {
            $videosController->newVideo();
        }
        elseif ( count($args) == 1 && preg_match( '/^\d+$/', $args[0] ) && $method == 'DELETE' )    // matches DELETE: /videos/:id
        {
            $videosController->delete( $args[0] );
        }
        elseif ( count($args) == 2 && preg_match( '/^\d+$/', $args[0] ) && $method == 'GET' )      // matches GET: /videos/:id/:action
        {
            switch ( $args[1] )
            {
                case 'flv':
